...
const accountsSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'accounts', 'index.ejs'), 'utf8');
const emailUpgradeSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'accounts', 'email-upgrade.ejs'), 'utf8');
const workflowsSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'workflows', 'index.ejs'), 'utf8');
const localJobsSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'local-jobs.ejs'), 'utf8');
const downloadsSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'downloads.ejs'), 'utf8');
const settingsSource = fs.readFileSync(path.join(__dirname, '..', 'views', 'settings.ejs'), 'utf8');
assert(serverSource.includes('browserAgentRequiredMessage'));
assert(serverSource.includes('requireConnectedBrowserAgent'));
assert(serverSource.includes('await requireConnectedBrowserAgent(userId, deviceId);'));
assert(!serverSource.includes('allow_website_only_browser_open'));
assert(!serverSource.includes('Website-only mode'));
assert(!dashboardSource.includes('Open in this browser'));
assert(!dashboardSource.includes('Open email signup page'));
assert(!dashboardSource.includes('Open account settings page'));
assert(!settingsSource.includes('Allow website-only normal browser open'));
assert(!settingsSource.includes('Require confirmation before direct/no-proxy open'));
assert(accountsSource.includes('Connected Browser Agent'));
assert(emailUpgradeSource.includes('Connect GS Browser Agent first'));
assert(emailUpgradeSource.includes('Queue Browser Agent Job'));
assert(workflowsSource.includes('Queue Browser Agent Job'));
assert(workflowsSource.includes('browserAgent.connected'));
assert(localJobsSource.includes('the website does not open browsers itself'));
assert(downloadsSource.includes('browser automation requires GS Browser Agent running on your Windows PC'));
...
async function main() {
  ...
  assert.strictEqual(testInternals.browserAgentRequiredMessage, 'GS Browser Agent is required to run browser automation. Download, open, and pair the agent first.');
  assert.strictEqual(testInternals.isBrowserAgentDevice({ device_role: 'agent_browser' }), true);
  assert.strictEqual(testInternals.isBrowserAgentDevice({ device_role: 'browser' }), true);
  assert.strictEqual(testInternals.isBrowserAgentDevice({ device_role: 'agent' }), false);
  ...
  assert.strictEqual(testInternals.jobTypeLabel('open_browser'), 'Browser Agent Open URL');
